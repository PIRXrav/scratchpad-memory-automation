...
def algo0(y, x, Dky, Dkx, dma):
    ...
    def compute(current_state, system_states, tensor_o, cost, dept, history_stack, best, state_eval_all):
        ...
        def heuristique(states):
            states = list(states)
            states.sort(key=lambda x: -np.sum(tensor_o.ravel()[state_eval_all[x[0]][x[1]]] != -1))
        ...
        for i, state in enumerate(progressbar(next_states, dept, tk)):
            ...
            if not is_not_match:  # This state has interest (NOT TRUE, but speed up a lot)
                ...
                compute(state, system_states - {state}, next_tensor_o, new_cost,
                        dept + 1, history_stack, best, state_eval_all)
    ...
    for comb in combination_dma_io:
        tensor_i_dma = dma_load(tensor_i, comb[0], dma)
        tensor_o_dma = dma_load(tensor_o, comb[1], dma)
    ...
    with enlighten.Manager() as manager:
        ...
        state_eval_all = [[list(state_eval((i, j))) for j in range(tensor_o.size - dma + 1)]
...
def algo0FullExploreNumba(y, x, Dky, Dkx, dma):
    ...
    def compute(current_state, system_states, system_states_iterator, system_states_iterator_row_col,
        ...
        # for i, state in enumerate(progressbar(next_states, dept, tk)):
        state_iterator = system_states_iterator if current_state[0] != -1 else \
            system_states_iterator_row_col[current_state[0]][current_state[1]]
    ...
    print(system_states)
    system_states_iterator = np.array(list(product(range(system_states.shape[0]),
                                                   range(system_states.shape[1]))), dtype=np.int32)
    print(f'{system_states_iterator.shape=} # {np.prod(system_states_iterator.shape)}')
    system_states_iterator_row_col = np.array([[list(rowcol_iterator(i, j, system_states))
                                              for i in range(tensor_i.size - dma + 1)]
                                              for j in range(tensor_o.size - dma + 1)], dtype=np.int32)
    # (tensor_i.size - DMA + 1,
    #  tensor_o.size - DMA + 1,
...
def algo1(y, x, Dky, Dkx, dma,
    ...
    def fast_explore():
        ...
        while not np.all(index_to_count_states == 0):
            ...
            #    print(s, -sum((index_to_count_states[k] != 0 and test_if_index_match_state(k, s)
            #               for k in range(tensor_o.size))))
    ...
    def fast_explore_numba(tensor_i, tensor_o, history_stack, progress_proxy):
        ...
        index_to_count_states = np.array([counte_states_as_index(tensor_i, tensor_o, ind_o)
                                         for ind_o in range(tensor_o.size)])
        # print(index_to_count_states)
    ...
    if v_heuristic:  # Generic version
        ...
        with enlighten.Manager() as manager:
            ...
            state_eval_all = [[set(state_eval((i, j))) for j in range(tensor_o.size - dma + 1)]
                              for i in range(tensor_i.size - dma + 1)]
            print('DONE')
            best = [99999, None]
            tk = progressbar_init(manager, [f"l{k}" for k in range(10)])
...
if __name__ == '__main__':
    ...
    if 0:
        start_time = time.time()
        best = algo0(y, x, Dky, Dkx, DMAA)
        print(best)
        # prog = export(best[1], tensor_i, tensor_o, DMAA)
        # prog.evaluate(DMAA)
        print("--- algo0 : %s seconds ---" % (time.time() - start_time))

    if 0:
        start_time = time.time()
        best = algo0FullExploreNumba(y, x, Dky, Dkx, DMAA)
    ...
    if 1:
        start_time = time.time()
        best = algo1(y, x, Dky, Dkx, DMAA, v_fast_explore=True, v_fast_explore_numba=True)
        # prog = export(best[1], tensor_i, tensor_o, DMAA)
        # prog.evaluate(DMAA)
